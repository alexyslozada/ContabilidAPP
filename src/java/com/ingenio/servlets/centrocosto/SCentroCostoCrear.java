...
@MultipartConfig
@WebServlet(name = "SCentroCostoCrear", urlPatterns = {"/SCentroCostoCrear"})
public class SCentroCostoCrear extends HttpServlet {

    private static final Logger LOG = Logger.getLogger(SCentroCostoCrear.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (Utilidades.get().autenticado(sesion)) {
            ...
            if (dao.tienePermiso(usuario.getPerfil(), dao.OBJETO, Constantes.INSERTAR)) {
                ...
                CentroCosto centrocosto = new CentroCosto(codigo, nombre);
                
                try{
                    short respuesta = dao.crear(centrocosto);
                    tipo = Constantes.MSG_CORRECTO;
                    mensaje = Constantes.MSG_CREADO_TEXT + respuesta;
                } catch (ExcepcionGeneral eg){
                    Utilidades.get().generaLogServer(LOG, Level.SEVERE, "Error en SCentroCostoCrear {0}", new Object[]{eg.getMessage()});
