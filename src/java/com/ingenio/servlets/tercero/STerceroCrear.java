...
@MultipartConfig
@WebServlet(name = "STerceroCrear", urlPatterns = {"/STerceroCrear"})
public class STerceroCrear extends HttpServlet {

    private static final Logger LOG = Logger.getLogger(STerceroCrear.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (Utilidades.get().autenticado(sesion)) {
            ...
            if (dao.tienePermiso(usuario.getPerfil(), dao.OBJETO, Constantes.INSERTAR)) {
                String tipoIdentificacion = request.getParameter("tipo_identificacion"),
                ...
                short iTipoIdentificacion, iTipoPersona, iCiudadResidencia;
                ...
                Tercero tercero = new Tercero(iTipoIdentificacion, numeroIdentificaci, digitoVerificacion, iTipoPersona, razonSocial, primerApellido, segundoApellido, primerNombre, segundoNombre, direccion, telefono, deptoResidencia, iCiudadResidencia, correo);
                
                try{
                    short respuesta = dao.crear(tercero);
                    tipo = Constantes.MSG_CORRECTO;
                    mensaje = Constantes.MSG_CREADO_TEXT + respuesta;
                } catch (ExcepcionGeneral eg){
                    Utilidades.get().generaLogServer(LOG, Level.SEVERE, "Error en STerceroCrear {0}", new Object[]{eg.getMessage()});
