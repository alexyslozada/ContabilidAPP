...
@MultipartConfig
@WebServlet(name = "STipoFuncionarioCrear", urlPatterns = {"/STipoFuncionarioCrear"})
public class STipoFuncionarioCrear extends HttpServlet {

    private static final Logger LOG = Logger.getLogger(STipoFuncionarioCrear.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(Utilidades.get().autenticado(sesion)){
            ...
            if(dao.tienePermiso(usuario.getPerfil(), dao.OBJETO, Constantes.INSERTAR)){
                ...
                tipoFuncionario.setNombre(nombre);
                
                try{
                    short respuesta = dao.crear(tipoFuncionario);
                    tipo = Constantes.MSG_CORRECTO;
                    mensaje = Constantes.MSG_CREADO_TEXT + respuesta;
                } catch (ExcepcionGeneral eg){
                    Utilidades.get().generaLogServer(LOG, Level.SEVERE, "Error en STipoFuncionarioCrear {0}", new Object[]{eg.getMessage()});
