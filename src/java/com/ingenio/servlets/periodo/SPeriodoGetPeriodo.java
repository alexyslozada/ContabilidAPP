...
@MultipartConfig
@WebServlet(name = "SPeriodoGetPeriodo", urlPatterns = {"/SPeriodoGetPeriodo"})
public class SPeriodoGetPeriodo extends HttpServlet {

    private static final Logger LOG = Logger.getLogger(SPeriodoGetPeriodo.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (Utilidades.get().autenticado(sesion)) {
            ...
            Periodo periodo = new Periodo(iAnio, iMes, true);

            try {
                short respuesta = dao.getPeriodo(periodo);
                tipo = Constantes.MSG_CORRECTO;
                mensaje = Constantes.MSG_CONSULTA_REALIZADA_TEXT;
                objeto = "{\"id_periodo_contable\":"+respuesta+"}";
            } catch (ExcepcionGeneral eg) {
                Utilidades.get().generaLogServer(LOG, Level.SEVERE, "Error en SPeriodoGetPeriodo {0}", new Object[]{eg.getMessage()});
