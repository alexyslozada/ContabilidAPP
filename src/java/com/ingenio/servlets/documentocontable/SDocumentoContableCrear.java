...
@MultipartConfig
@WebServlet(name = "SDocumentoContableCrear", urlPatterns = {"/SDocumentoContableCrear"})
public class SDocumentoContableCrear extends HttpServlet {

    private static final Logger LOG = Logger.getLogger(SDocumentoContableCrear.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (Utilidades.get().autenticado(sesion)) {
            ...
            if (dao.tienePermiso(usuario.getPerfil(), dao.OBJETO, Constantes.INSERTAR)) {
                ...
                DocumentoContable documentoContable = new DocumentoContable(abreviatura, documento);
                try{
                    short respuesta = dao.crear(documentoContable);
                    tipo = Constantes.MSG_CORRECTO;
                    mensaje = Constantes.MSG_CREADO_TEXT + respuesta;
                } catch (ExcepcionGeneral eg){
                    Utilidades.get().generaLogServer(LOG, Level.SEVERE, "Error en SDocumentoContableCrear {0}", new Object[]{eg.getMessage()});
