...
@MultipartConfig
@WebServlet(name = "SDocumentoContableEliminar", urlPatterns = {"/SDocumentoContableEliminar"})
public class SDocumentoContableEliminar extends HttpServlet {

    private static final Logger LOG = Logger.getLogger(SDocumentoContableEliminar.class.getName());
        ...
        if (Utilidades.get().autenticado(sesion)) {
            ...
            if (dao.tienePermiso(usuario.getPerfil(), dao.OBJETO, Constantes.BORRAR)) {
                ...
                try{
                    ...
                    Utilidades.get().generaLogServer(LOG, Level.SEVERE, "Error en SDocumentoContableEliminar {0}", new Object[]{eg.getMessage()});
                    tipo = Constantes.MSG_ERROR;
