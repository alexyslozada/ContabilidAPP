...
@MultipartConfig
@WebServlet(name = "SDocumentoContableActualizar", urlPatterns = {"/SDocumentoContableActualizar"})
public class SDocumentoContableActualizar extends HttpServlet {

    private static final Logger LOG = Logger.getLogger(SDocumentoContableActualizar.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (Utilidades.get().autenticado(sesion)) {
            ...
            if (dao.tienePermiso(usuario.getPerfil(), dao.OBJETO, Constantes.MODIFICAR)) {
                ...
                DocumentoContable documentoContable = new DocumentoContable(iId, abreviatura, documento);
                try{
                    boolean respuesta = dao.actualizar(documentoContable);
                    if(respuesta){
                    ...
                    Utilidades.get().generaLogServer(LOG, Level.SEVERE, "Error en SDocumentoContableActualizar {0}", new Object[]{eg.getMessage()});
                    tipo = Constantes.MSG_ERROR;
