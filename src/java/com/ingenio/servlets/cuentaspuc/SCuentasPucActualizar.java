...
@MultipartConfig
@WebServlet(name = "SCuentasPucActualizar", urlPatterns = {"/SCuentasPucActualizar"})
public class SCuentasPucActualizar extends HttpServlet {

    private static final Logger LOG = Logger.getLogger(SCuentasPucActualizar.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (Utilidades.get().autenticado(sesion)) {
            ...
            if (dao.tienePermiso(usuario.getPerfil(), dao.OBJETO, Constantes.MODIFICAR)) {
                ...
                CuentasPuc cuentasPuc = new CuentasPuc(iId, nombre, bCentroCosto);
                try{
                    boolean respuesta = dao.actualizar(cuentasPuc);
                    if(respuesta){
                    ...
                    Utilidades.get().generaLogServer(LOG, Level.SEVERE, "Error en SCuentasPucActualizar {0}", new Object[]{eg.getMessage()});
