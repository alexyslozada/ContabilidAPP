...
@MultipartConfig
@WebServlet(name = "SEmpresaFuncionarioEliminar", urlPatterns = {"/SEmpresaFuncionarioEliminar"})
public class SEmpresaFuncionarioEliminar extends HttpServlet {

    private static final Logger LOG = Logger.getLogger(SEmpresaFuncionarioEliminar.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(Utilidades.get().autenticado(sesion)){
            ...
            if(dao.tienePermiso(usuario.getPerfil(), dao.OBJETO, Constantes.BORRAR)){
                ...
                short iIdFuncionario;
                
                iIdFuncionario = Utilidades.get().parseShort(sIdFuncionario, LOG, true);
                
                try{
                    boolean respuesta = dao.eliminar(iIdFuncionario);
                    if(respuesta){
